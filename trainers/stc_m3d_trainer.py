...
class STC_M3D_Trainer(object):
    def __init__(self, rank, config, model, logit_scale, pc_adapter, view_weights_module, view_fusion_module,optimizer,scheduler, train_loader, modelnet40_loader, objaverse_lvis_loader, scanobjectnn_loader):
        self.rank = rank
        self.config = config
        self.model = model
        ...
        self.best_modelnet40_acc = 0
        self.modelnet40_best_epoch = 0
    ...
    def test_modelnet40(self):
        ...
        if self.rank == 0:
            ...
            if overall_acc > self.best_modelnet40_acc:
                self.best_modelnet40_acc = overall_acc
                self.modelnet40_best_epoch = self.epoch
                self.save_model('best_modelnet40')

            logging.info(f'Test ModelNet40: overall acc: {overall_acc}')
            logging.info(
